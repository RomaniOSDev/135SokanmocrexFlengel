...
@MainActor
final class MixedChallengeViewModel: ObservableObject {
    ...
    init(difficulty: Difficulty) {
        ...
        self.startedAt = Date()
        self.secondsRemaining = difficulty.timedQuestionSeconds + (difficulty == .easy ? 5 : 0)
        self.items = Self.buildMixedDeck()
    }

    private static func buildMixedDeck() -> [TriviaItem] {
        var pool: [TriviaItem] = []
        for lv in 1...ActivityKind.levelCount {
            pool.append(contentsOf: LandmarkQuestionData.items(for: lv).prefix(2))
            pool.append(contentsOf: CulturalQuestionData.items(for: lv).prefix(2))
            pool.append(contentsOf: HistoricalQuestionData.items(for: lv).prefix(2))
        }
    ...
    func advance() {
        ...
        secondsRemaining = difficulty.timedQuestionSeconds + (difficulty == .easy ? 5 : 0)
        timerCancellable?.cancel()
        timerCancellable = nil
        startTimerIfNeeded()
    }
