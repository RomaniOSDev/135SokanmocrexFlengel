//
//  CulturalConundrumsViewModel.swift
...
@MainActor
final class CulturalConundrumsViewModel: ObservableObject {
    let level: Int
    ...
    init(level: Int, difficulty: Difficulty) {
        ...
        self.items = CulturalQuestionData.items(for: level)
        self.livesRemaining = difficulty.lives
        self.startedAt = Date()
        if difficulty != .easy {
            secondsRemaining = difficulty.timedQuestionSeconds
        }
    }
    ...
    func startTimerIfNeeded() {
        guard usesTimer, timerCancellable == nil, !isFinished else { return }
